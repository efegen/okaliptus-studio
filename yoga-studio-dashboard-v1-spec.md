...
- Reaksiyon sayaçları içerik durumu olarak yalnız mevcutsa gövdenin altında kompakt bir şeritte görünür; `Yanıtla` ve ikon + `Tepki` sabit eylem satırındadır. Not sahibinin `Düzenle`/`Sil` eylemleri sağdaki `•••` menüsündedir. Emoji seçici ve işlem menüsü dışarı dokunma veya Escape ile kapanır. Yeni not composer'ında hatırlatıcı: alt panelde hızlı zamanlar (Yarın / 1 hafta sonra / 1 ay sonra — üçü de saat 12:00'a sabit), özel tarih/saat ve çoklu ekip alıcısı; seçim form üzerinde özetlenip düzenlenebilir/kaldırılabilir.
- Hatırlatıcı kalıcıdır ve gerçekten bildirim gönderir (migration `0282_note_reminders.sql`): `note_reminders` tablosu not başına tek seferlik bir hatırlatma satırı tutar (`remind_at`, `recipient_user_ids`, tek `sent_at` damgası — tüm alıcılara aynı anda gider). `POST /notes` gövdesi opsiyonel `reminder: { remindAt, recipientUserIds }` kabul eder; zaman geçmişteyse veya alıcı listesi boş/pasifse not hiç oluşmadan reddedilir (aynı transaction). Alıcı seçici `GET /notes/reminder-recipients`'ten gelir — `/users`'ın aksine (yalnız owner) tüm roller erişebilir. Gönderim `notification-scheduler.ts`'teki `checkNoteReminders` ile diğer bildirim türleriyle aynı tick'te işlenir; başlık/metin şablonu (`{author}`, `{note}`) Ayarlar → Bildirimler'de `note_reminder` anahtarıyla owner tarafından ayarlanabilir (bu türün GLOBAL alıcısı yoktur — "Kimler alsın" kartı gizlidir, alıcılar her hatırlatmayla birlikte seçilir).
- Migration `0278_note_categories.sql`: stüdyo genelinde ortak `note_categories` katalogunu ve üst seviye notlardaki nullable `category_id` bağlantısını ekler. Mobil Notlar başlığındaki ayarlar düğmesiyle tüm roller kategori ekleyebilir, yeniden adlandırabilir ve silebilir. Silme notları kaldırmaz; `ON DELETE SET NULL` ile yalnız kategori bağlantısını temizler. Kategoriler yeni notta seçilir, mevcut not düzenlenirken değiştirilebilir ve akışta rozet + filtre olarak görünür. Uçlar: `GET/POST /notes/categories`, `PATCH/DELETE /notes/categories/:categoryId`; not `POST/PATCH` gövdesi opsiyonel `categoryId` kabul eder. Kategori adı stüdyo genelinde büyük/küçük harf duyarsız benzersiz ve en fazla 40 karakterdir; yanıtlar kategori taşımaz.

#### Etkinlik katılımcı iletişim aksiyonları (2026-09-06)

Mobil etkinlik katılımcı listesinde satırı sola kaydırmak **Kaldır**, sağa kaydırmak **Arandı** aksiyonunu açar. Her iki aksiyon da doğrudan veri değiştirmek yerine bir alt panel açar. Kaldırmada neden seçimi zorunlu (`student_cancelled`, `plans_changed`, `added_by_mistake`, `other`), not isteğe bağlıdır. Aramada not isteğe bağlıdır; kayıtlı telefon varsa panel `tel:` kısayolu sunar. Arama geçmişi katılımcı satırında toplam sayıyla birlikte **ARANDI · N** rozetiyle görünür. Hiç tamamlanmış dersi olmayan kişi **YENİ ÖĞRENCİ** rozeti alır. Katılımcı satırının sağında ücret yerine RSVP durumu (**Geliyor** / **Belirsiz**) gösterilir; ücret ayrıntısı katılımcı detay ekranında kalır.

Migration `0277_event_participant_interactions.sql`, arama ve kaldırma kayıtlarını `event_participant_interactions` tablosunda append-only operasyon geçmişi olarak saklar. `participant_id` bilinçli olarak FK değildir; böylece listeden kaldırılan katılımcının nedeni/notu kaybolmaz. Notlar en fazla 500 karakterdir. Yeni arama aksiyonu audit'te `event_participant_contacted`, kaldırma ise mevcut `event_participant_removed` olarak izlenir.

#### Misafirli katılımcı kaldırma seçenekleri (2026-09-06)

Misafiri olan biri (host) `removeParticipant`'a `guestResolution` belirtilmeden gönderilirse eskisi gibi `EVENT_PARTICIPANT_HAS_GUESTS` (409) ile reddedilir. Mobil kaldırma paneli bu durumu önceden bilir (katılımcı listesi zaten önbellekte) ve neden seçiminin altında ikinci, zorunlu bir soru daha açar: **"[Ad] N misafiri var — [misafir adları] ne olsun?"** — iki seçenekle:

- **Bağlantıları kopart** (`guestResolution: 'unlink'`) — misafirler silinmez, yalnız `guest_of_participant_id` `NULL`'lanır; bundan sonra herkes gibi bağımsız, normal katılımcı olurlar. Kendi RSVP/rol/ödeme/ulaşım durumları aynen kalır. Her misafir için `event_participant_guest_unlinked` audit satırı yazılır (yeni action, migration `0281`).
- **Misafirleri de kaldır** (`guestResolution: 'remove_guests'`) — host'un kendisiyle birlikte tüm misafirleri de listeden çıkarılır; host silinmiyorsa da gelmeyecekse misafiri de gelmez varsayımıyla. Önce TÜM misafirlerin tahsilatı kontrol edilir — biri bile ödeme almışsa hiçbiri silinmez, `EVENT_PARTICIPANT_HAS_PAYMENTS` (409) döner (kısmi/tutarsız bir liste bırakmamak için). Geçerse her misafir host ile aynı kaldırma nedeni/notuyla `event_participant_interactions`'a yazılır ve mevcut `event_participant_removed` action'ı tekrar kullanılır (yeni action gerekmez).

Panelde gönderim, neden seçilene VE (misafir varsa) bu ikinci seçim yapılana kadar kapalı kalır — sessiz varsayım yok, kullanıcı sonucu açıkça seçmek zorunda. Seçime tıklanınca ne olacağını anlatan kısa bir açıklama (misafir adlarıyla) altında belirir. `EventParticipantActionSheet` (`src/mobile/events/EventParticipantActionSheet.jsx`) hem etkinlik listesinden hem katılımcı profilinden aynı bileşeni kullandığından iki ekran da bu akışı otomatik alır. Hareketler akışında `event_participant_guest_unlinked` için geri alma tanımlı DEĞİL (host aynı anda kaldırıldığından geri bağlanacak bir satır kalmaz) — diğer geri alınamaz kayıtlar gibi "Bu hareket türünün otomatik geri alması yok" gösterilir.

#### Ulaşım ekranı kompakt düzeni (2026-09-06)

Mobil **Ulaşım** ekranı yalnız yerleşim düzeyinde yeniden kurgulandı; veri modeli, uç noktalar ve atama akışı aynıdır. Amaç, çok araçlı bir etkinlikte tüm planı daha az kaydırmayla görebilmek.

- Gövdenin üstünde yapışkan bir sayaç şeridi var: **araç · koltuk (dolu/toplam) · bekliyor · araç dışı**. `bekliyor` sıfırdan büyükken amber tonuna geçer; kaydırma sırasında ekranda kalır.
- **Araç bekleyenler** tam genişlik satır listesi yerine iki sütunlu kutucuk ızgarasıdır — kimse ekran dışında kalmadan yarı yükseklikte listelenir.
- Araç kartında şoför + yolcular tek bir sarmalanan çip şerididir (eski tam genişlik satır listesi yerine). Buluşma yeri, saat ve `rental_service` için **Servis** etiketi başlık altındaki tek meta satırında birleşti; koltuk pimleri metinsiz kaldı, sayısal karşılık rozette (`n boş` / `Dolu`) durur.
- Kart üzerindeki kalıcı **Düzenle/Sil** buton satırı kaldırıldı; başlıktaki `⋯` düğmesi şoför/telefon/koltuk/buluşma/not alanlarını ve **Sil**'i içeren tek paneli açar. Silme, araçta yolcu varken kapalı kalmaya devam eder.
- **Kendi gelenler** ve **Ulaşımı seçilmemiş** tek bir "Araç dışındakiler" kartında iki etiketli çip grubuna toplandı; belirsiz olanlar kesikli çerçeveyle ayrılır.
- Araç seçme panelinde araç satırları iki satırdan tek satıra indi (`buluşma · tip · dolu/toplam` + rozet) ve mevcut araç en üstte, kalanlar boş koltuğu çoktan aza sıralanır. Kişi bir araçtayken panel, durum seçmenin onu araçtan çıkaracağını yazar.

#### Etkinlik hareketleri ve geri alma (2026-09-06)

Mobil etkinlik detayındaki kısayol şeridine **Notlar** ile **Finans** arasına **Hareketler** eklendi. Ekran, o etkinlikte yapılan işlemleri kim yaptıysa adıyla, en yeniden eskiye ve gün başlıklarıyla listeler; hatalı bir işlemi geri aldırır. Kısayol şeridi artık daima kaydırılabilir moddadır (`.evx-scroller.is-overflowing`) — yeni kısayol mevcut butonları daraltmaz, her çip aynı genişlikte kalır ve bir seferde üçü tam görünür.

Akış ayrı bir tablo değildir: `audit_logs`'un bu etkinliğe bağlı satırlarıdır. Migration `0279_event_activity.sql` şunları ekler: `audit_logs.event_id` (FK `events`, `ON DELETE SET NULL` — denetim kaydı silinmesin, yalnız akıştan düşsün) + kısmi indeks, `reverted_at`/`reverted_by_user_id` damgaları, iki yeni action (`event_participant_contact_reverted`, `event_participant_vehicle_unassigned`) ve mevcut etkinlik kayıtlarının tek seferlik backfill'i. `event_id` join ile türetilmez, çünkü kaldırılan katılımcının satırı fiziksel olarak silinir ve o kayıt hiçbir etkinliğe bağlanamazdı.

**Geri alma bir silme değildir.** Sunucu telafi işlemini normal servis yolundan yapar (aynı doğrulamalar, aynı kilitler, kendi audit kaydı) ve orijinal satır yalnız "geri alındı" damgası alır; böylece hem hata hem düzeltmesi geçmişte görünür kalır ve aynı kayıt iki kez geri alınamaz. Sıra bilinçlidir: önce telafi, sonra damga.

- **Geri alınabilir:** etkinlik bilgisi güncelleme, katılımcı ekleme (→ kaldırma) / kaldırma (→ eski rol, RSVP, misafir bağı ve ulaşım tercihiyle geri ekleme) / güncelleme (→ eski değer) / "arandı" kaydı (→ etkileşim satırının silinmesi), ücret kapsamı ve kişiye özel ders ücreti, tahsilat (→ iptal, defter bozulmadan), araç ekleme/güncelleme/silme, araca bindirme.
- **Geri alınamaz** (neden kullanıcıya Türkçe gösterilir): etkinlik oluşturma ve silme (Ayarlar → Durum'dan yeniden açılır), ücret kalemi ekleme, tahsilat iptali, misafir bağlantısı koparma (`event_participant_guest_unlinked` — host aynı işlemde kaldırıldığından geri bağlanacak satır kalmaz), zaten geri alınmış kayıtlar ve 0279 öncesinde "önce" değeri saklanmadan yazılmış kayıtlar.
- Telafi işlemi iş kurallarını atlamaz: ödemesi alınmış ya da misafiri olan katılımcının eklenmesi geri alınamaz, dolu araca geri bindirme reddedilir, kontenjanı aşan geri alma reddedilir.
- Kayıt hâlâ listede olan bir kişiye işaret ediyorsa alt panelde **Kişiyi aç** ile katılımcı detayına gidilip düzeltme elle de yapılabilir.

Uçlar: `GET /events/:id/activity` (opsiyonel `limit`, varsayılan 100, en çok 300), `POST /events/:id/activity/:activityId/revert`. İkisi de `requireCan('audit.read')` ile kapılıdır — akış tahsilat tutarı içerdiğinden **asistan rolü göremez**; mobilde kısayol da aynı yetkiyle gizlenir (kozmetik; güvenlik sunucuda). Smoke: `42-event-activity.ts`.

Kayıt kalitesi için iki servis yazımı genişletildi: `updateParticipant` artık yalnız gönderilen alanların "önce" değerini, `assignParticipantToVehicle` ise önceki araç + ulaşım tercihini audit'e yazar; `markParticipantContacted` sildiği satırı kesin bulabilmek için etkileşim kimliğini kaydeder (zaman damgası JSON'a milisaniye hassasiyetiyle düştüğü için eşleşme güvenilir değildi).

#### Katılımcı profili not günlüğü (2026-09-06)

Katılımcı detay ekranındaki tekil, üzerine yazılan "Bu etkinlik için not" alanı (`event_participants.note`) kaldırıldı; yerine genel "Notlar" akışının küçültülmüş bir hâli geldi: birden fazla kullanıcı ekleyebilir, her not yazarının adıyla görünür, yalnız yazar kendi notunu düzenleyip **kalıcı** silebilir (soft-delete yok — yanıt zinciri gibi korunması gereken bir yapı olmadığından). Yanıt/"@" bahis/tepki/fotoğraf/kategori bilinçli olarak yok; kapsam "ekle, yazarı görünsün, kendi yazdığını düzenle/sil" ile sınırlı. Bu notlar yalnız o kişinin o etkinlikteki profilinde görünür — stüdyo geneli Notlar akışına karışmaz.

Migration `0280_event_participant_notes.sql`: yeni `event_participant_notes` tablosu (`participant_id` → `event_participants` `ON DELETE CASCADE`, `author_user_id` → `users`, `body`, `created_at`, `updated_at`) + `event_participants.note` kolonunun düşürülmesi + üç yeni audit action'ı (`event_participant_note_created/updated/deleted`) ve `event_participant_note` entity_type'ı. Uçlar: `GET/POST /events/participants/:participantId/notes`, `PATCH/DELETE /events/participant-notes/:noteId` (tekil not uçları flat — `/events/payments/:paymentId` örüntüsünü izler). Bu yazımlar "Hareketler" akışına (audit_logs.event_id) bilinçli olarak sızmaz — genel Notlar'ın kendisi de o akışta görünmez, tutarlılık korunur. Smoke: `40-events.ts` bölüm S.

#### Etkinlik günü ekranı (2026-09-12)

Etkinlik detayındaki **"Etkinlik gününü başlat"** artık çalışır ve kapıda kullanılan tek bir ekran açar. Tasarım hedefi hızdır: kalabalıkta kişi başına birkaç dokunuş. Eski katılımcı listesi, ücret kalemleri ve tahsilat defteri (0261/0263/0276) bu ekranda **kullanılmaz ve değişmez**; önceden girilen ön ücretler burada anlam taşımaz. Esas olan etkinlik günü gerçekten alınan paradır.

**Kurallar (kullanıcı kararları):**
- Listeye yazılan herkes gelmiş sayılır; geldi/gelmedi alanı yok.
- Satır başına: kişi, alınan tutar (elle girilir; **"Tam ödeme"** tek dokunuşla doldurur), ödeme yöntemi (nakit / kart / IBAN; tutar 0 ise yöntem yok), kahvaltı durumu, isteğe bağlı not.
- Kahvaltı: yok / **bize ödedi** / **restorana ödedi** / **bedava**. "Restorana ödedi" bilgi amaçlıdır: kartla ödeyen, dersi stüdyonun POS'undan öder, kahvaltıyı restorana kendisi öder ve restoranın fişini getirince kahvaltı fişini alır. Nakit/IBAN seçilince kahvaltı "bize ödedi", kart seçilince "restorana ödedi" varsayılanını alır (kullanıcı kahvaltı seçimini elle değiştirmediyse). Restoranın ücretsiz kahvaltı kontenjanı hesaba katılmaz.
- **Kahvaltı yarım ödenmez:** kahvaltı bize ödendiyse tutar en az kahvaltı fiyatı olmalıdır; ilk kahvaltı fiyatı kadarı kahvaltı, kalanı ders sayılır. İndirim ve bedava yalnız ders kısmındadır; bedava kahvaltı ayrı "bedava" durumudur (ücret alınmaz ama kahvaltı listesine girer).
- "Tam ödeme" = ders ücreti + (kahvaltı bize ödendiyse) kahvaltı ücreti. Fiyatlar etkinliğin ücret kalemlerinden gelir: ders = `is_lesson_fee` kalemi (yoksa ilk geçiş-dışı kalem), kahvaltı = ilk `is_pass_through` kalemi.
- Tutarlar her zaman düzenlenebilir; kilit, kesin onay veya etkinliği kapatma yok. Her satırda bir **"ödeme OK" tiki** var: kilitlemez, geri alınabilir, kimin tiklediği saklanır; tutar, yöntem veya kahvaltı değişince kendiliğinden kalkar ("bu rakama baktım" anlamı rakam değişince geçersizleşir).
- Silme onay ister ve soft-delete'tir; ekranda birkaç saniye **"Geri al"** sunulur.
- Aynı öğrenci aynı etkinliğe iki kez yazılamaz (DB kısmi unique index — iki telefon aynı anda eklese bile).
- Kapıda eklenen yeni kişi **hafif kayıttır**: ana öğrenci listesine düşmez (`student_id` boş). Ad soyad zorunlu, telefon isteğe bağlı; telefon alanı `0 (5__) ___ __ __` kalıbıyla yalnız 5 ile başlayan 10 haneli TR cep numarası kabul eder, serbest metin değildir. Yazılan numara kayıtlı bir öğrencide ya da başka bir hafif kayıtta varsa uyarı çıkar ve o kişi seçilebilir.
- Arama: isim (Türkçe karakter duyarsız — "ayse" → "Ayşe"; yazılan kelimelerin hepsi geçmeli), telefonun tamamı/başı veya son 4 hanesi. DB `LC_CTYPE 'C'` olabildiğinden katlama `lower()` yerine önce `translate()` ile yapılır. Öğrenciler ve bugünkü hafif kayıtlar birlikte aranır; ön kayıtlılar (eski katılımcı listesindekiler) "ön kayıtlı" etiketiyle öne çıkar; zaten listede olan kişi "listede" görünür ve dokununca mevcut satır düzenlenir. Telefonla aranıp bulunamayan kişi yeni kayıt olarak eklenirken numara alana hazır gelir.
- Özet (yalnız bu ekranda): **kasa** (nakit / kart / IBAN / toplam ve toplamın restorana ait kahvaltı kısmı), **kahvaltı** (fiş sayısı; bize ödeyen / restorana ödeyen / bedava; restorana ödenecek = (bize ödeyen + bedava) × kahvaltı fiyatı), kişi ve kontrol edilen sayısı. Özet rakamlarına dokunmak listeyi süzer.
- Tüm roller kullanır; asistan toplamlar dahil her şeyi görür. KPI / ciro / Hareketler'e hiçbir şey yansımaz.
- Liste 5 saniyede bir kendini yeniler (birden fazla telefonla aynı anda kullanım).

**Başlatma:** "Etkinlik gününü başlat" `upcoming` etkinliği `live`'a çeker ve ekranı açar; `live` iken buton "Etkinlik gününe dön" olur ve ana sayfadaki etkinlik kartı da doğrudan bu ekranı açar. Tamamlanmış/iptal etkinlikte buton "Etkinlik günü kayıtları" olur ve durumu değiştirmez. Açık ekranın etkinlik kimliği `localStorage`'da (`okaliptus-event-day-id`) tutulur; telefon sekmeyi kapatıp yeniden yüklese de kullanıcı aynı ekrana döner.

**Şema (migration `0284_event_day_entries.sql`):** `event_day_entries` — `event_id`, `student_id` (nullable, `ON DELETE SET NULL`: öğrenci kalıcı silinirse satır kendi ad kopyasıyla kalır), `full_name` (bağlı öğrencide görüntü canlı öğrenci adından gelir), `phone` (yalnız hafif kayıtta, `^5[0-9]{9}$`), `amount` (≥ 0), `payment_method` (`cash`/`card`/`iban`; `(amount > 0) = (payment_method IS NOT NULL)` CHECK), `breakfast` (`none`/`paid_to_us`/`paid_to_restaurant`/`free`), `note`, `checked_at` + `checked_by_user_id`, `created_by`/`updated_by`/`deleted_by` kullanıcıları, `created_at`/`updated_at`/`deleted_at`. Kısmi unique index `(event_id, student_id) WHERE deleted_at IS NULL AND student_id IS NOT NULL`. Audit: `event_day_entry_created/updated/deleted/restored` + `event_day_entry` entity_type; kayıtlar etkinlik hareket akışına (`audit_logs.event_id`) bilinçli olarak bağlanmaz.

**Uçlar:** `GET /events/:id/day` (etkinlik, fiyatlar, özet, satırlar), `GET /events/:id/day/search?q=`, `POST /events/:id/day/entries` (gövde `studentId` **veya** `fullName` + `phone`; `amount`, `paymentMethod`, `breakfast`, `note`, `checked`), `PATCH /events/day-entries/:entryId` (aynı alanlar; ad/telefon yalnız hafif kayıtta), `DELETE /events/day-entries/:entryId` (soft), `POST /events/day-entries/:entryId/restore`. Hepsi tüm rollere açık. Servis: `backend/src/services/event-day.service.ts`. Smoke: `43-event-day.ts`. Mobil: `src/mobile/events/MobileEventDay.jsx`, `EventDayEntrySheet.jsx`, `eventDayUtils.js`.

**Bu turun kapsamı dışında:** ciro/KPI yansıması, eski katılımcı modülüyle birleştirme, hafif kaydı sonradan öğrenciye çevirme, toplu ödeme, etkinlik öncesi ödeme.
