-- 0273'te notlar etkinlikten koparıldı (event_notes → notes); notes.service.ts
-- artık 'note_created' / 'note_updated' / 'note_deleted' action'larıyla ve 'note'
-- entity_type'ıyla insertAuditLog çağırıyor. Eski migration'lar in-place
-- düzenlenmez; bu dosya 0271'deki (action) ve 0269'daki (entity_type) tam
-- listeleri korur + genişletir.
--
-- Eski 'event_note_*' değerleri listede KALIR: 0273 öncesinde yazılmış audit
-- satırları hâlâ o değerleri taşıyor, CHECK eklenirken mevcut satırlar da
-- doğrulanıyor.
...
ALTER TABLE audit_logs ADD CONSTRAINT audit_logs_action_check
  CHECK (action IN (
    ...
    'note_updated',
    'note_deleted',
    -- 0273 öncesi (etkinliğe bağlı notlar) — geçmiş satırlar için korunuyor.
    'event_note_created',
    'event_note_updated',
    'event_note_deleted',
...
ALTER TABLE audit_logs ADD CONSTRAINT audit_logs_entity_type_check
  CHECK (entity_type IN (
    ...
    'event',
    'note',
    -- 0273 öncesi — geçmiş satırlar için korunuyor.
    'event_note',
    'event_fee_item',
    'event_participant',
